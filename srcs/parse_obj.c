...
void		ft_parse_sq(char *line, t_mini_rt *rt)
{
	...
	sq->id = 3;
	rt->line_xyz = ft_split(rt->split[1], ',');
	sq->vec = parse_vec(rt);
	clear_arr(&rt->line_xyz);
	check_arg((rt->line_xyz = ft_split(rt->split[2], ',')), rt);
	...
	valid_d(&sq->d, rt);
	check_arg((rt->line_rgb = ft_split(rt->split[4], ',')), rt);
...
void		ft_parse_cy(char *line, t_mini_rt *rt)
{
	...
	valid_d(&cy->h, rt);
	check_arg((rt->line_rgb = ft_split(rt->split[5], ',')), rt);
...
void		ft_parse_tr(char *line, t_mini_rt *rt)
{
	...
	tr->id = 5;
	check_arg((rt->line_xyz = ft_split(rt->split[0], ',')), rt);
	tr->vec = parse_vec(rt);
	clear_arr(&rt->line_xyz);
	check_arg((rt->line_xyz = ft_split(rt->split[1], ',')), rt);
